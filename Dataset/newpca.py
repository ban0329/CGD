...
#save_path='D:/SSP-mainS/data/train_features/'
...
def wtn_feature(feature_path):
    """
    Load features from pickle file and train PCA whitening.
    
    Args:
        feature_path: Path to the feature pickle file
    """
    # Load features from the provided path
    features = np.ascontiguousarray((load_pickle(feature_path))['train'])
    features = torch.tensor(features).float()
    pca_wtn = PCA(dim1=2048, dim2=2048)
    return pca_wtn.train_pca(features)
    #save_pickle(save_path,pca_wtn)
