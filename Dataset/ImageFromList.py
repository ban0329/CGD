...
class SfM120k(data.Dataset):
    def __init__(self, imsize=512, mode='train', data_root=None):
        super().__init__()

        # SFM images
        ims_root = os.path.join(data_root, "ims")
        db_fn = os.path.join(data_root, "retrieval-SfM-120k.pkl")
...
class GLDv2(data.Dataset):
    def __init__(self, imsize=1024, prefix=None,feature_path=None):
        super().__init__()
        ...
        self.transforms = transforms.Compose([
            transforms.RandomResizedCrop(size=imsize, scale=(0.6, 1.0)),
...
#     GLDv2_build_train_dataset(csv_path=r"google-landmark-master/train.csv",clean_csv_path=r"google-landmark-master/train_clean.csv",image_dir=r"google-landmark-master/train",output_directory="./",generate_train_validation_splits=True,validation_split_size=0.01,seed=17)