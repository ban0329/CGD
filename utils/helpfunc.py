...
def extract_vectors(net, loader, ms=[1], device=torch.device('cuda')):
    net.eval()
    vecs = torch.zeros(len(loader), 512, device=device)
    if len(ms) == 1:
...
def get_root():
    """
    Get root directory. This function should be configured via environment variable
    or replaced with a proper configuration system.
    For now, returns None to force explicit path specification.
    """
    # Return None to force explicit path usage
    # Users should set paths via command line arguments
    return None


def get_data_root():
    """
    Get data root directory. Returns None to force explicit path specification.
    """
    root = get_root()
    if root is None:
        return None
